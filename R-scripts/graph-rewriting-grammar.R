...
modRemoveEdge <- function(graphobject){
	...
	newgraph <- removeEdge(from=nodelist[edgeframe$from[chosenedge]], 
						  ...
						  graph=graphobject)
	cat("removing edge: ", nodelist[edgeframe$from[chosenedge]], "-->", 
...
modDupNode <- function(graphobject){
	...
	cat("duplicating node: ", node.to.duplicate, "\n")
	return(newgraph)
}
...
evolveDAG <- function(nmods, startgraph){
	...
	for (i in 1:nmods){
		...
		if (whichmod==1){
#			cat("removing a node\n")
			evgraph <- modRemoveNode(evgraph)
		} else if (whichmod==2){
#			cat("removing an edge\n")
			evgraph <- modRemoveEdge(evgraph)
		} else if (whichmod==3){
#			cat("duplicating a node\n")
			evgraph <- modDupNode(evgraph)
		} else if (whichmod==4){
#			cat("adding an edge\n")
			evgraph <- modAddEdge(evgraph)
		}
	}
	cat("\n")
	return(evgraph)
