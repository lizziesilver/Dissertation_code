...
################################################################################
get_pag_degree_stats <- function(truepag){
  gu <- .jnew("edu/cmu/tetrad/graph/GraphUtils$ConnectivityMeasurements", truepag)
  
  deg_stats <- vector(mode="numeric", length=16)
  deg_stats[1] <- gu$getMaxDegree()
  deg_stats[2] <- gu$getMaxIndegree() 
  deg_stats[3] <- gu$getMaxOutdegree()
  deg_stats[4] <- gu$getMaxAmbdegree()
  deg_stats[5] <- gu$getMinDegree()
  deg_stats[6] <- gu$getMinIndegree()
  deg_stats[7] <- gu$getMinOutdegree()
  deg_stats[8] <- gu$getMinAmbdegree()
  
  deg_stats[9] <- gu$getAvgDegree()
  deg_stats[10] <- gu$getStdDegree()
  deg_stats[11] <- gu$getAvgIndegree()
  deg_stats[12] <- gu$getStdIndegree()
  deg_stats[13] <- gu$getAvgOutdegree()
  deg_stats[14] <- gu$getStdOutdegree()
  deg_stats[15] <- gu$getAvgAmbdegree()
  deg_stats[16] <- gu$getStdAmbdegree()
  
  names(deg_stats) <- c("maxDegree", "maxIndegree",
                        "maxOutdegree", "maxAmbdegree",
                        "minDegree", "minIndegree",
                        "minOutdegree", "minAmbdegree",
                        "avgDegree", "stdDegree",
                        "avgIndegree", "stdIndegree", 
                        "avgOutdegree", "stdOutdegree",
                        "avgAmbdegree", "stdAmbdegree")
...
LearnDirected <- function(specificAlgorithms, newindtest_fisherz, newcovmat,
  ...
  if (specificAlgorithms == "ges"){
    # run GES
    newges <- .jnew("edu/cmu/tetrad/search/Fgs", newcovmat)
  ...
  if (specificAlgorithms == "fci"){
    ...
    newfci$setVerbose(TRUE)
    learnedgraph <- newfci$search()
  }
  
  # RFCI
  if (specificAlgorithms == "rfci"){
    # run RFCI
    newfci <- .jnew("edu/cmu/tetrad/search/Rfci", newindtest_fisherz)
    newfci$setMaxPathLength(as.integer(maxPathLength))
    newfci$setVerbose(TRUE)
    learnedgraph <- newfci$search()
  }
