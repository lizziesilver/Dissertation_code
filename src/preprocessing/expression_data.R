...
mus1 <- read.csv("Raw/MESCs/KleinData/GSE65525_RAW/GSM1599494_ES_d0_main.csv", header=FALSE)
...
write.csv(mus_subset_data, "Processed/MESCs/mus_subset_data.csv", row.names=F)
...
write(sample_size, "Processed/MESCs/cor_pearson_subset_with_header.txt", append=TRUE)
write(gene_subset, "Processed/MESCs/cor_pearson_subset_with_header.txt", 
      ncolumns=length(gene_subset), sep="\t", append=TRUE)
# Append the correlation matrix to file
write(cor_pearson_subset, "Processed/MESCs/cor_pearson_subset_with_header.txt", 
      ncolumns=length(gene_subset), sep="\t", append=TRUE)

# make correlation matrix: SKEPTIC estimator of correlation
cor_skeptic_subset <- huge.npn(mus_subset_data, npn.func = "skeptic")
# create the header that Tetrad needs: sample size and col names
write(sample_size, "Processed/MESCs/cor_skeptic_subset_with_header.txt", append=TRUE)
write(gene_subset, "Processed/MESCs/cor_skeptic_subset_with_header.txt", 
      ncolumns=length(gene_subset), sep="\t", append=TRUE)
# Append the correlation matrix to file
write(cor_skeptic_subset, "Processed/MESCs/cor_skeptic_subset_with_header.txt", 
      ncolumns=length(gene_subset), sep="\t", append=TRUE)
